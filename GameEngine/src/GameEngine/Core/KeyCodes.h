...
namespace GameEngine
{
	...
	inline std::string format_as(const KeyCode& keyCode) {
		return std::to_string(static_cast<int32_t>(keyCode));
	}
...
template<>
struct std::formatter<GameEngine::KeyCode, char> {
	constexpr auto parse(std::format_parse_context& ctx) {
		return ctx.begin();
	}

	auto format(const GameEngine::KeyCode& kc, std::format_context& ctx) const {
		std::formatter<int> int_formatter;
		return int_formatter.format(static_cast<int32_t>(kc), ctx);
	}
};

...
namespace GameEngine {
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		if (in)
		{
			...
			if (size != -1)
			{
				...
				GE_CORE_ERROR("Could not read from file '{}'", filepath);
		}
		else
		{
			GE_CORE_WARN("Could not open file '{}'", filepath);
		}
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		GE_CORE_ASSERT(shaderSources.size() <= 2, "We only support 2 shaders for now (vertex + fragment)");
		...
		for (auto& kv : shaderSources)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				GE_CORE_ASSERT(false, "{} shader compilation failure!", StringFromShaderType(type));
				break;
			}
